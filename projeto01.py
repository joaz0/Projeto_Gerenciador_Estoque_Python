...
def adicionar_item():  
    ...
    if item_vet in itens_saida1:    
        try :  
            label = customtkinter.CTkLabel(rolagem_saida2, text=item_vet)            
            label.grid(row=linha, column=0, pady=5, padx=5, sticky = "w")     
            lixeira = customtkinter.CTkButton(rolagem_saida2, width=35, height=35, text="", image=imagem1, command=lambda: delete_itens(label, lixeira))
