...
def adicionar_item():
    global linha
    item_vet = str(nome_ret.get())
    linha += 1

    if item_vet in itens_saida1:
        try:
            label = customtkinter.CTkLabel(rolagem_saida2, text=item_vet, anchor="w")
            label.grid(row=linha, column=0, pady=5, padx=5)
            lixeira = customtkinter.CTkButton(rolagem_saida2, width=35, height=35, text="", image=imagem1, command=lambda: delete_itens(label))
            lixeira.grid(row=linha, column=1, pady=5, padx=5)
        except ValueError:
            return
...
label_do_cadastro.grid(row=0, column=1, padx=5, pady=5)
...
label_edicao.grid(row=0, column=0, padx=5, pady=10, sticky="n", columnspan=4)
...
for item in items:
    Box = customtkinter.CTkCheckBox(rolagem, text=items, text_color="#8684EB", checkmark_color="#83A2EB", border_color="#83A2EB")
...
label_saida.grid(row=0, column=0, columnspan=2, padx=10, pady=10, sticky="n")
...
for item1 in itens_saida1:
    box_saida1 = customtkinter.CTkCheckBox(rolagem_saida1, text=itens_saida1, text_color="#8684EB", checkmark_color="#83A2EB", border_color="#83A2EB", border_width=2)
...
# botões
butao_add_item = customtkinter.CTkButton(quadro_saida, text="Adicionar item", text_color="black", width=110, fg_color="#83A2EB", command=adicionar_item)
butao_add_item.grid(row=3, column=2, columnspan=2, pady=5, sticky="n")
...
# frame 5
label_entrada = customtkinter.CTkLabel(quadro_entrada, text="entrada", font=("arial", 20, "bold"), text_color="#8684EB")
label_entrada.grid(row=0, column=2, padx=10, pady=10, sticky="e")

# frame 6
label_relatorio = customtkinter.CTkLabel(quadro_relatorio, text="relatório", font=("arial", 20, "bold"), text_color="#8684EB")
label_relatorio.grid(row=0, column=2, padx=10, pady=10, sticky="e")
